...
const SCENARIOS: Record<
  string,
  { reply: string; steps: { label: string; tool: string; result?: string; hitl?: boolean }[] }
> = {
  file: {
    reply: "On it. I'll search your files and summarize what I find.",
    steps: [
      {
        label: 'Scanning Documents for recent files',
        tool: 'find_file',
        result: 'Found 14 files modified in the last 7 days'
      },
      {
        label: 'Reading Sales_Report_Q2.pdf',
        tool: 'read_pdf',
        result:
      ...
      },
      {
        ...
        tool: 'gemini',
        result: 'Summary generated and copied to clipboard.'
      }
    ]
  },
  web: {
...
export function handleDemoApproval(messageId: string, stepId: string): void {
  const store = useTorchStore.getState()
  store.updateStep(messageId, stepId, {
    status: 'done',
